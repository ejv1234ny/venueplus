...
- **Home** — teal→coral hero with the pill search bar, venue-type grid, featured venues, and the service-category cards.
...
## Built from
`Card` (interactive hover-lift), `Badge` (category + status), `Button` (coral "Book Now" CTA), `Input`, `Select`, `Tabs`, `Checkbox`.

Venue imagery uses the app's authentic teal→coral gradient fallback (the real product's empty-photo state).
